...
if ! wp core is-installed --allow-root --path="$ROOT" >/dev/null 2>&1; then
  wp core install --allow-root --path="$ROOT" --url="$SITE_URL" --title="کلینیک زیبایی و پوست زوشا" \
    --admin_user="${WP_ADMIN_USER:?WP_ADMIN_USER is required}" --admin_password="${WP_ADMIN_PASSWORD:?WP_ADMIN_PASSWORD is required}" \
    --admin_email="${WP_ADMIN_EMAIL:?WP_ADMIN_EMAIL is required}" --skip-email
fi
...
echo "ZOSHA_BOOTSTRAP: design=5.0.0"
echo "ZOSHA_BOOTSTRAP: ready $SITE_URL"
PORT_NUM="${PORT:-8080}"
PUBLIC_HOST="${RAILWAY_PUBLIC_DOMAIN:-wordpress-v5-production.up.railway.app}"
(
 sleep 4
 ...
 HERO_COUNT="$(printf '%s' "$HOME_HTML" | grep -o 'hero-v5' | wc -l | tr -d ' ')"
 IMG_COUNT="$(printf '%s' "$HOME_HTML" | grep -o '/wp-content/uploads/' | wc -l | tr -d ' ')"
 echo "ZOSHA_QA: hero-v5=${HERO_COUNT}"
